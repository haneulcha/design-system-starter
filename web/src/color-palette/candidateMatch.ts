...
// 상한을 정확히 10에 두는 근거는 "두 자릿수부터"류의 눈대중이 아니라 이 파일
// 위쪽 실측 표에 이미 있는 값이다: 6액센트×stop[0,3,7,10] 중 노랑 stop 950의
// 최인접 후보쌍 거리가 정확히 체비셰프 10이다 — 우리 데이터 안에서 이미
// 근접으로 다룬 실측치를 그대로 상한으로 쓰는 것이지 임의의 반올림이 아니다.
// 그 안쪽은 "같은 자리를 가리키는 후보들이 서로 가까운" 정상 범위이고, 11
// 이상(1.9%)부터는 최댓값 55(`#e2e236` stop 300: 적용색 #f4f23c vs 승자
// #f1f173)까지 벌어져 눈에 띄게 다른 색을 "지금 적용 중"이라고 가리키는 셈이다.
// 색 도구가 그러면 안 된다: 아무것도 안 보여주는 건 정직하고, 틀린 걸 보여주는
// 건 거짓말이다. 그래서 상한을 넘으면 null(체크 없음)을 낸다 — 화면은 그
// null을 "지금 색은 이 앵커의 후보에 없습니다"로 말한다(CandidatePopover
// 참조). 이 자리는 Task 10의 D6 pin-복원("복원된 pin은 새 액센트 후보 어디에도
// 없다") 시나리오가 그대로 올라탈 자리이기도 하다 — 스펙이 예고한 "후보 밖"
// 상태가 여기다.
