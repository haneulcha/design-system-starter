...
describe("pickCurrent (argmin, D7-1 C-1 수정)", () => {
  ...
  // 2026-08-30 재리뷰(R-2): argmin이 무조건 승자를 내면, target이 사실 그
  // 어떤 후보와도 안 닮았을 때도 "가장 덜 먼" 후보를 "지금 적용 중"으로
  // 거짓 표시한다. 실측(액센트 180종 스윕, 80,932건)으로 승자 거리 상한을
  // 10으로 뒀다 — 10 이하가 98.1%, 11 이상부터 눈에 띄게 달라진다(최대 55).
  it("MAX_DISTANCE(10) 안이면 고른다", () => {
    // #f5f5f5 vs #ecece2: R 9, G 9, B 19 → 최대 19라 이 케이스 자체는 상한
    // 밖이다. 상한 안쪽 경계값으로 확인한다: 거리 정확히 10.
    expect(pickCurrent(["#101010"], "#1a1a1a")).toBe("#101010"); // 거리 10
  });

  it("MAX_DISTANCE(10)를 넘으면 null — 실측 사례(액센트 #e2e236 stop 300)", () => {
    // 문맥이 바뀐 뒤 재계산된 이 stop의 후보 셋(#edefa2 거리102, #eff18c
    // 거리80, #f0f174 거리56)이 target(#f4f23c)과 전부 56 이상 떨어져 있다 —
    // 직접 candidatesFor로 확인한 실제 값. argmin이라면 #f0f174(가장 가까움,
    // 56)를 골랐겠지만, 상한을 넘으므로 null이어야 한다.
    expect(pickCurrent(["#edefa2", "#eff18c", "#f0f174"], "#f4f23c")).toBeNull();
  });

  it("경계값: 거리 11이면 null(10은 포함, 11부터 제외)", () => {
    expect(pickCurrent(["#101010"], "#1b1b1b")).toBeNull(); // 거리 11
  });
});
