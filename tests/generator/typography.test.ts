...
describe("generateTypography", () => {
  it("generates 14 hierarchy styles", () => {
    const t = generateTypography(getArchetype("professional"), "Inter");
  ...
  it("applies archetype heading weight", () => {
    ...
    expect(generateTypography(getArchetype("professional"), "X").hierarchy[0].weight).toBe(400);
    expect(generateTypography(getArchetype("clean-minimal"), "X").hierarchy[0].weight).toBe(400);
  });
