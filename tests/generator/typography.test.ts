...
describe("generateTypography", () => {
  it("generates 14 hierarchy styles", () => {
    const t = generateTypography(getArchetype("precise"), "Inter");
  ...
  it("applies archetype heading weight", () => {
    expect(generateTypography(getArchetype("confident"), "X").hierarchy[0].weight).toBe(700);
    expect(generateTypography(getArchetype("precise"), "X").hierarchy[0].weight).toBe(400);
    expect(generateTypography(getArchetype("modern"), "X").hierarchy[0].weight).toBe(400);
  });

  it("uses provided font family", () => {
    const t = generateTypography(getArchetype("expressive"), "DM Sans");
