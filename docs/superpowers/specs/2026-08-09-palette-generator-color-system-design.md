...
- **시맨틱 채도를 액센트에 맞추기** — 뮤트한 브랜드에 tailwind 채도 그대로의
  시맨틱이 튀는지. 현재는 앵커 고정.
- ~~뉴트럴 어트랙터 직접 선택~~ — 해소: `2026-08-15-color-palette-generator-design.md`
  Task 10 (`5cafae6`/`5af5189`, 2026-08-16). `/color-palette`의 `NeutralControl`이
  칩 5개 중 직접 선택 + "자동으로" 되돌리기 버튼을 준다.
