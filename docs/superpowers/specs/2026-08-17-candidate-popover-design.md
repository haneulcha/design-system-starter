...
| 닫힘 | Esc · 바깥 `pointerdown` · 후보 확정 — 포커스가 패널 안에 있었으면 트리거로 복귀 (예외는 아래) |
...
닫힘 시 포커스 복귀는 원칙적으로 **모든 경로**다. Radix의 `onCloseAutoFocus` 기본 동작이
그렇고(바깥 클릭에서 복귀를 막으려면 오히려 `preventDefault`가 필요한 쪽이다), 실제로
라디오를 고르면 그 노드가 언마운트되므로 복귀가 없으면 포커스가 `body`로 떨어진다.

**예외 하나 — 빈 영역을 마우스로 클릭해 닫는 경우 포커스는 `body`로 간다.** 구현 후
실브라우저 검증에서 확인했다. 우리 `pointerdown` 핸들러가 트리거로 복귀시킨 **뒤에**,
브라우저의 `mousedown` 기본 동작이 포커스를 옮긴다 — 클릭 지점이 포커스 불가 요소면
`body`가 된다. 합성 `pointerdown`만 쏘면 복귀가 정상 동작하므로 로직 자체는 맞고,
실브라우저의 이벤트 순서에 지는 것이다. jsdom은 기본 동작이 없어 이 경로를 영원히
재현하지 못한다 — 테스트가 초록인 것이 보증이 되지 않는 자리다.

고치려면 복귀를 `requestAnimationFrame`으로 미루거나 바깥 `pointerdown`에
`preventDefault`를 걸어야 하는데, 전자는 경쟁 조건에 패치를 대는 것이고 후자는 텍스트
선택 같은 다른 기본 동작까지 죽인다. **남기기로 했다** — 키보드 사용자의 경로인 Esc와
포커스 가능한 요소를 누르는 경로는 정상이고, 빈 곳을 눌렀을 때 포커스가 `body`로 가는
것은 네이티브 UI 다수의 행동과도 같다.
...
패널을 중앙 정렬하면 stop 950 칸에서 오른쪽으로 넘친다. 뷰포트 가장자리는 한참 멀어서
**뷰포트 clamp는 발동조차 하지 않고**, 패널이 `gap-8`을 넘어 sticky `PreviewPane`을
덮는다. 그 목업이 hover 프리뷰를 실시간으로 보는 대상이라, 가리면 이 도구의 루프를
정면으로 거스른다.

*(구현 후 실측: 띠 608px · 칸 53.45px · 패널 113.5px. stop 950에서 clamp가 30px
되밀어 패널 우변이 띠 우변과 정확히 맞고, 화살표 중심은 트리거 중심과 0.1px 이내로
일치한다. 설계 당시 패널 폭을 ~170px로 어림했는데 실제는 그보다 좁았다 — 결론은
같다. clamp 없이는 여전히 넘친다.)*
...
4. **패널 폭이 칸보다 넓다.** 53px 칸 아래 ~114px 패널이라 좌우 이웃 칸을 덮는다.
   화살표가 앵커를 가리키므로 어느 stop인지는 읽히지만, 덮인 이웃 색은 패널을 닫아야
   다시 보인다.
5. **빈 영역을 마우스로 클릭해 닫으면 포커스가 `body`로 간다.** 위 D2의 예외 항목 참조 —
   브라우저의 `mousedown` 기본 동작이 우리 복귀 뒤에 오기 때문이고, jsdom은 이 경로를
   재현하지 못한다. Esc와 포커스 가능한 요소 클릭은 정상 복귀한다.
6. **Tab으로 패널 밖에 나가도 프리뷰가 안 걷힌다.** 마우스는 `onMouseLeave`가 걷지만
   키보드에는 대응물이 없어, 띠와 목업이 마지막으로 본 후보 색으로 남는다. 확정
   팔레트(`scales`)는 오염되지 않고 닫으면 원래대로 돌아온다.
