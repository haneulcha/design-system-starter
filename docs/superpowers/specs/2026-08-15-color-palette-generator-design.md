...
- `#builder`를 없애거나 바꾸지 않는다. 학습 화면으로 그대로 남는다.
...
**`border`는 검사하지 않는다.** 측정값이 어떤 앵커에서도 1.19~1.36으로 3:1을 한 번도
...
`assertColorSystem`은 `kind`에 따라 갈라져 검사한다: `stop`은 인덱스 범위,
`contrast`는 `against`가 실재하는 `stop` 역할 id인지.
...
**이동의 수명:** 적용된 이동은 명시적 값으로 상태에 남고, 이후 액센트를 바꿔도 유지된다
...
```
- `--color-warning-text`는 라이트 테마에서 본문 크기 AA(4.5:1)에 미달한다(2.96).
  큰 글씨(18.66px 이상 또는 14px 굵게)에만 쓰거나 `--color-warning-text-strong`을 쓸 것.
```
...
**이동 검증** — `builder`·`neutral`·`semantic`·`roles` 테스트가 **수정 없이** 통과.
깨지면 이동 중 무언가 잘못된 것이다.
...
- 같은 시스템 안에서 스케일마다 `on-solid` 값이 다를 수 있다 (뉴트럴 흰색 / 액센트 검정)
- `assertColorSystem`이 `against`가 없는 역할 id를 가리키면 던진다
- 기존 `stop` 역할만 있는 시스템이 그대로 통과한다 (하위 호환)
...
3. **조정 가능한 stop은 4개 고정이다** (50·300·700·950). 임의 stop 미세조정은 이월.
4. **Figma 별칭은 여전히 없다.** `on-solid`도 해석된 hex로 나간다.
5. **`/color-palette`가 정적 호스팅에서 rewrite를 요구한다.** 현재 배포 설정이 없어
   문제가 드러나지 않는다.
6. **되돌리기가 stop 단위다.** 전체 실행 취소 스택은 없다.
