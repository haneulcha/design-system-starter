...
> **2026-08-30 부분 개정** — `2026-08-30-color-palette-ux-repair-design.md`의
> 전용 절("사이클 3 D9의 부분 개정")이 이 규범을 다시 손봤다. 뒤집은 것은
> "교보재는 전부 안 싣는다"라는 일반 명제가 아니라 그 경계선이다: **"왜 이 색이
> 좋은가"(학습)는 계속 안 싣지만 "왜 이건 못 바꾸나 / 방금 무엇이 바뀌었나"(조작
> 사실)는 싣는다.** 그 경계로 D4의 "고칠 수 없다" 사유, D5의 이동 요약, D7-2의
> "후보가 겹쳐 하나만 남았다" 안내가 새로 실렸다. **원문의 핵심 결정(단계별
> 설명 문단·후보 note·"내가 고른 여정" 요약을 안 싣는다, `#builder`가 학습
> 역할을 진다)은 그대로다** — 좁아진 것은 "교보재"와 "조작 사실"이 뭉뚱그려져
> 있던 경계뿐이다.
